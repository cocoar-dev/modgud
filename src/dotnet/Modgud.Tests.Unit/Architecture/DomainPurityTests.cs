...
/// <summary>
/// <c>Modgud.Domain</c> holds events, value objects, and event-sourced
...
/// <list type="bullet">
///   <item><description>Domain → Modgud.Authorization (legacy project
...
public class DomainPurityTests
{
    ...
    public void Domain_should_not_depend_on_AspNetCore()
    {
        ...
        Assert.True(
            ...
            TestResultFormatter.Format(result,
                "Modgud.Domain must not depend on ASP.NET Core — " +
    ...
    public void Domain_should_not_depend_on_OpenIddict()
    {
        ...
        Assert.True(
            result.IsSuccessful,
            TestResultFormatter.Format(result,
                "Modgud.Domain must not depend on OpenIddict — " +
