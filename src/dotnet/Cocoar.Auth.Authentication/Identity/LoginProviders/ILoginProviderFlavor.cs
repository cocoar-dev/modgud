...
namespace Cocoar.Auth.Authentication.Identity.LoginProviders;

/// <summary>
/// Contract every external-login-provider flavor implements. The flavor
/// encapsulates everything that differs between providers (Entra, Okta,
/// Google, ...) — endpoint derivation, default scopes, default claims-transform
/// script, admin-UI schema, and sensible raw-claims-storage default.
/// <para>
/// Flavors are DI-registered as singletons and collected by
/// <see cref="LoginProviderFlavorRegistry"/>. The key returned by <see cref="Key"/>
/// is the stable string stored on <c>LoginProvider.Flavor</c> — immutable after
/// creation.
/// </para>
/// <para>
/// Built-in <c>Internal</c> providers do not have a flavor — runtime code
/// short-circuits on <c>LoginProvider.Type == Internal</c> rather than
/// resolving a flavor.
/// </para>
/// </summary>
public interface ILoginProviderFlavor
{
    /// <summary>Stable key (see <c>Cocoar.Auth.Authentication.Domain.LoginProviders.LoginProviderFlavor</c> for canonical values).</summary>
    string Key { get; }
    ...
    /// <summary>
    /// Default for <c>LoginProvider.StoreRawClaims</c> — enterprise flavors
    /// (Entra, Okta, Keycloak) default to <c>true</c> because claim-debugging
    /// is mission-critical; consumer flavors (Google, GitHub) default to
