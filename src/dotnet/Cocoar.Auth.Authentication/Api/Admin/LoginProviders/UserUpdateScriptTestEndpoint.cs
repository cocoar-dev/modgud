...
namespace Cocoar.Auth.Authentication.Api.Admin.LoginProviders;

/// <summary>
/// Admin helpers for authoring the per-LoginProvider user-update script: run
/// a test evaluation against sample claims, and load the last raw claims that
/// the IdP sent during a real login so the admin can iterate without
/// triggering end-user logins.
/// </summary>
    ...
    public static void MapUserUpdateScriptTestEndpoint(this IEndpointRouteBuilder endpoints, string path)
    {
        ...
        group.MapPost("{id}/test-user-update",
                   ...
                   CancellationToken ct) =>
            {
                ...
                {
                    var config = await session.LoadAsync<LoginProvider>(id.Guid, ct);
        ...
        // Load the last raw claims from the most recent login via this provider.
        ...
        group.MapGet("{id}/last-raw-claims",
            async (ShortGuid id,
                   ...
                   CancellationToken ct) =>
            {
                var providerId = id.Guid;
                var link = await session.Query<ExternalIdentityLink>()
                    .Where(l => l.LoginProviderId == providerId && !l.IsUnlinked && l.LastRawClaims != null)
