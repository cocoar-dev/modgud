...
/// <summary>
/// Central OpenTelemetry meters for Modgud domain events. Picked up
/// by the OTel wiring via <c>.AddMeter(ModgudMeters.Name)</c>, so any
...
/// </summary>
public static class ModgudMeters
{
    ...
    private static readonly Counter<long> _twoFactorBlocked = Meter.CreateCounter<long>(
        "modgud.two_factor.enforcement.blocked.total",
        unit: "{request}",
    ...
    private static readonly Counter<long> _dcrRateLimit = Meter.CreateCounter<long>(
        "modgud.dcr.rate_limit.hit.total",
        unit: "{hit}",
