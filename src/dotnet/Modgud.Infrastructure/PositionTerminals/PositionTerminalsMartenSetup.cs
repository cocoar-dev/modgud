...
/// <summary>
/// Marten wiring for the position-terminals slice (MG-FT work-item series):
/// documents, inline projections, and stable event-type aliases. Grows with the
/// series (TerminalEnrollment, StaffingCeremony, StaffingSession follow
/// in MG-FT-03/05).
/// </summary>
public static class PositionTerminalsMartenSetup
{
    public static StoreOptions UseModgudPositionTerminals(this StoreOptions options)
    {
        // Grant documents — inline projection of the grant streams. Indexes per
        // plan §5.2: the per-position list, the per-user lookup, the status
        // filter, and the composite the uniqueness check runs on.
        options.Schema.For<PositionGrant>()
            .Identity(x => x.Id)
            .Index(x => x.PositionPrincipalId)
            .Index(x => x.UserId)
            .Index(x => x.Status)
            .Index(x => new { x.PositionPrincipalId, x.UserId });

        options.Projections.Add<PositionGrantProjection>(ProjectionLifecycle.Inline);
        ...
        // half is the position-terminal client invariant). NOTE deliberate
        ...
        options.Schema.For<StaffingCeremony>()
            .Identity(x => x.Id)
        ...
        options.Events.MapEventType<PositionGrantSuspended>("position_grant_suspended");
        options.Events.MapEventType<PositionGrantResumed>("position_grant_resumed");
        options.Events.MapEventType<PositionGrantRevoked>("position_grant_revoked");
