...
/// <summary>
/// Ends function staffing sessions outside their natural token flow (MG-FT-07,
/// plan §15.1): local/remote lock and every revocation cascade (user, passkey,
/// grant, terminal, function) funnel through here. Each operation selects only
...
/// </summary>
public interface IFunctionStaffingRevoker
{
    ...
    Task<int> EndAllForFunctionAsync(Guid functionId, StaffingSessionEndReason reason, CancellationToken ct = default);

    Task<int> EndAllForUserAndFunctionAsync(Guid userId, Guid functionId, StaffingSessionEndReason reason, CancellationToken ct = default);
