...
/// <c>resource_access[<see cref="CocoarAuthOptions.Audience"/>]</c> from
    ...
    public CocoarAuthClaimsTransformation(IOptions<CocoarAuthOptions> options)
    {
        ...
        if (string.IsNullOrWhiteSpace(_options.Audience))
            throw new InvalidOperationException(
                "CocoarAuthOptions.Audience must be set to the resource server's audience " +
                "(same value as JwtBearerOptions.Audience). Configure it via AddCocoarAuthClient.");
    }
