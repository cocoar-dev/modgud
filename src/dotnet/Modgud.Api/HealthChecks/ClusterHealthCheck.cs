...
///   <item>more than one node is alive but this host runs without a SignalR
///   backplane (only possible outside Production) → 503 with a message that
///   names the fix. Pushes raised on one node would never reach connections on
///   the other, which is a functional break, not a warning.</item>
/// </list>
    ...
    ClusterHostingOptions hosting) : IHealthCheck
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var data = new Dictionary<string, object>
        {
            ...
            ["backplane"] = hosting.Backplane.ToString(),
        };

        if (live.Count > 1 && hosting.Backplane == ClusterBackplane.None)
        {
            return HealthCheckResult.Unhealthy(
                $"{live.Count} nodes are alive but this host runs without a SignalR backplane. " +
        ...
        return HealthCheckResult.Healthy(
            live.Count == 1 ? "Single live node." : $"{live.Count} live nodes, {hosting.Backplane} backplane active.",
            data);
    }
}
