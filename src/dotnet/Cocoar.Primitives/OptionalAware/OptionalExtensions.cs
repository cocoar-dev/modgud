...
namespace Cocoar.Primitives.OptionalAware
{
    public static class OptionalExtensions
    {
        ...
        ///// <summary>
        ///// Map TIn → TOut, where your projector returns a plain Task<TOut>.
        ///// Exceptions become Error.Failure; success is wrapped in Optional.
        ///// </summary>
        //public static async Task<ErrorOr<Optional<TOut>>>
        //    MapErrorOrAsync<TIn, TOut>(
        //        this Optional<TIn> opt,
        //        Func<TIn, Task<TOut>> projector)
        //{
        //    if (!opt.HasValue)
        ...
        //    try
        //    {
        //        var value = await projector(opt.Value!);
        ...
        /// <summary>
        /// Bind TIn → TOut, where your binder returns an ErrorOr&lt;TOut&gt;.
        /// Exceptions become Error.Failure; binder‑errors are propagated; success is wrapped in Optional.
        /// </summary>
        public static async Task<ErrorOr<Optional<TOut>>>
            MapErrorOrAsync<TIn, TOut>(
                this Optional<TIn> opt,
                Func<TIn, Task<ErrorOr<TOut>>> binder)
        {
            if (!opt.HasValue)
                return Optional<TOut>.None;
            ...
            try
            {
                var result = await binder(opt.Value!);
                if (result.IsError)
                    return result.Errors;     // implicit ErrorOr conversion
                return new Optional<TOut>(result.Value);
            }
            catch (Exception ex)
            {
                return Error.Failure(ex.Message);
