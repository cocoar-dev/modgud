...
/// proof, an admin holding <c>function-terminal:enroll</c> sees the TERMINAL
/// consent (function, slot, key fingerprint) and approves, and the device's
...
public class TerminalDeviceEnrollmentTests : IntegrationTestBase
{
    ...
    public async Task A_pending_slot_enrolls_via_the_dpop_bound_device_flow()
    {
        ...
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-enroll", terminalEnabled: true, ct);
        ...
        Assert.Equal("fn-enroll", consent.GetProperty("FunctionName").GetString());
    ...
    public async Task Approval_is_refused_without_the_enroll_permission()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-noperm", terminalEnabled: true, ct);
    ...
    public async Task An_unbound_device_request_cannot_reach_an_approval()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-unbound", terminalEnabled: true, ct);
    ...
    public async Task A_wrong_key_poll_does_not_enroll_and_the_right_key_still_can()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-wrongkey", terminalEnabled: true, ct);
    ...
    public async Task An_enrolled_slot_cannot_be_enrolled_again()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-reenroll", terminalEnabled: true, ct);
    ...
    public async Task Terminal_verification_is_dark_while_the_flag_is_off()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-dark", terminalEnabled: true, ct);
