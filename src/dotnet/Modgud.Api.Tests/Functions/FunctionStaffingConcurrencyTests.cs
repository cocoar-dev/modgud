...
public class FunctionStaffingConcurrencyTests : IntegrationTestBase
{
    public FunctionStaffingConcurrencyTests(SharedPostgresFixture fixture) : base(fixture) { }
    ...
    // ─── scenario setup + flow helpers (shared shape with FunctionStaffingTests) ──
    ...
    private sealed record StaffingSetup(
        ...
        FunctionStaffingTests.DpopProofBuilder DeviceKey,
    ...
    private async Task<StaffingSetup> SetUpEnrolledTerminalWithGrantedUserAsync(string accountName, CancellationToken ct)
    {
        ...
        var deviceKey = new FunctionStaffingTests.DpopProofBuilder();
    ...
    private async Task<BeginResult> BeginStaffingAsync(StaffingSetup setup, CancellationToken ct)
    {
        ...
        request.Headers.Add(DpopConstants.HeaderName,
            setup.DeviceKey.CreateProof("POST", "http://localhost/connect/function-staffing/begin", DateTimeOffset.UtcNow));
    ...
    private async Task<HttpResponseMessage> PostTokenAsync(Dictionary<string, string> form, FunctionStaffingTests.DpopProofBuilder key)
    {
