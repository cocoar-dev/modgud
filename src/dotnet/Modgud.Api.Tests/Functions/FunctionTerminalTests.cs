...
public class FunctionTerminalTests : IntegrationTestBase
{
    public FunctionTerminalTests(SharedPostgresFixture fixture) : base(fixture) { }
    ...
    public async Task Terminals_are_dark_while_the_feature_flag_is_off()
    {
        ...
        Assert.Equal(HttpStatusCode.NotFound, (await Client.GetAsync($"/api/function/{anyId}/terminals", ct)).StatusCode);
    }
    ...
    public async Task Slot_create_requires_the_terminal_policy_to_be_enabled()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-policy-off", terminalEnabled: false, ct);

        var resp = await Client.PostAsJsonAsync($"/api/function/{fn}/terminals",
    ...
    public async Task A_slot_creates_the_managed_public_client_atomically_with_the_fixed_profile()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-slot", terminalEnabled: true, ct);
        ...
        Assert.Equal(new ShortGuid(terminal.FunctionId).Guid, client.LinkedFunctionPrincipalId);
        ...
        [
            "gt:refresh_token",
            "gt:" + FunctionGrantTypes.StaffingSession,
    ...
    public async Task The_generic_oauth_admin_surface_is_read_only_for_terminal_clients()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-locked", terminalEnabled: true, ct);
        ...
        Assert.Contains("owned by a function terminal", await put.Content.ReadAsStringAsync(ct));

        var delete = await Client.DeleteAsync($"/api/admin/oauth/clients/{adminId}", ct);
        Assert.False(delete.IsSuccessStatusCode);
        Assert.Contains("owned by a function terminal", await delete.Content.ReadAsStringAsync(ct));
    }
    ...
    public async Task The_lifecycle_is_idempotent_and_revoked_is_terminal()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-lifecycle-t", terminalEnabled: true, ct);
        ...
        var reject = await Client.PostAsync($"/api/function/{fn}/terminals/{terminal.Id}/disable", null, ct);
    ...
    public async Task Two_slots_get_two_distinct_clients()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-two-slots", terminalEnabled: true, ct);
        ...
        var list = await Client.GetFromJsonAsync<List<TerminalDto>>($"/api/function/{fn}/terminals", JsonOptions, ct);
    ...
    public async Task Slots_are_event_sourced_one_event_per_transition()
    {
        var ct = TestContext.Current.CancellationToken;
        SetFeatureFlag(true);
        var fn = await CreateFunctionAsync("fn-slot-stream", terminalEnabled: true, ct);
    ...
    public async Task A_zero_role_user_gets_403()
    {
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await zeroClient.GetAsync($"/api/function/{anyId}/terminals", ct)).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await zeroClient.PostAsJsonAsync($"/api/function/{anyId}/terminals",
            new { DisplayName = "x", WebAuthnRpId = RpId }, JsonOptions, ct)).StatusCode);
    }

    // ─── helpers ──────────────────────────────────────────────────────────

    private async Task<string> CreateFunctionAsync(string accountName, bool terminalEnabled, CancellationToken ct)
    {
        var resp = await Client.PostAsJsonAsync("/api/function", new
        {
    ...
    private async Task<TerminalDto> CreateTerminalAsync(string functionId, string displayName, CancellationToken ct)
    {
        var resp = await Client.PostAsJsonAsync($"/api/function/{functionId}/terminals",
    ...
    private async Task<TerminalDto> TransitionAsync(string functionId, string terminalId, string action, CancellationToken ct)
    {
        var resp = await Client.PostAsync($"/api/function/{functionId}/terminals/{terminalId}/{action}", null, ct);
        var body = await resp.Content.ReadAsStringAsync(ct);
