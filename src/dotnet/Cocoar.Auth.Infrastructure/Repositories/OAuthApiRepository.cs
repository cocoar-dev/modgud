...
/// <summary>
/// Marten-based repository for OAuth APIs.
...
public class OAuthApiRepository : IOAuthApiRepository
{
	...
	public OAuthApiRepository(ITenantSessionFactory sessionFactory)
	{
		_sessionFactory = sessionFactory;
	}

	public async Task<OAuthApiListDto> GetAllAsync(
		PaginationRequest pagination,
		CancellationToken cancellationToken = default)
	{
		await using var session = _sessionFactory.OpenQuerySession();

		var query = session.Query<OAuthApiState>()
			.Where(x => !x.IsDeleted);
	...
	public async Task<OAuthApiDto?> GetByIdAsync(
		string id,
		CancellationToken cancellationToken = default)
	{
		...
		await using var session = _sessionFactory.OpenQuerySession();
		var resource = await session.LoadAsync<OAuthApiState>(guid, cancellationToken);
	...
	public async Task<ErrorOr<OAuthApiCreatedDto>> CreateAsync(
		...
		CancellationToken cancellationToken = default)
	{
		...
		var (_, createdEvent) = OAuthApiAggregate.Create(
			id,
			dto.Name,
			dto.DisplayName,
			dto.Description,
		...
		session.Events.StartStream<OAuthApiAggregate>(id, createdEvent);

		// Emit user claims event if specified
		if (dto.UserClaims.Count > 0)
		{
			session.Events.Append(id, new OAuthApiUserClaimsChanged(id, dto.UserClaims));
		}

		// Generate and store API secret
		var apiSecret = GenerateSecret();
		var securityData = OAuthApiSecurityData.Create(id);
	...
	public async Task<ErrorOr<OAuthApiDto>> UpdateAsync(
		string id,
		...
		CancellationToken cancellationToken = default)
	{
		...
		var currentState = await session.LoadAsync<OAuthApiState>(guid, cancellationToken);
		if (currentState is null || currentState.IsDeleted)
		{
			return OAuthErrors.ApiNotFound(id);
		}

		var aggregate = await session.Events.AggregateStreamAsync<OAuthApiAggregate>(guid, token: cancellationToken);
		...
		var updatedState = await session.LoadAsync<OAuthApiState>(guid, cancellationToken);
		...
		CancellationToken cancellationToken = default)
	{
		...
		session.Delete<OAuthApiSecurityData>(guid);
		...
		CancellationToken cancellationToken = default)
	{
		...
		var securityData = await session.LoadAsync<OAuthApiSecurityData>(guid, cancellationToken)
		...
		CancellationToken cancellationToken = default)
	{
		...
		var resource = await session.Query<OAuthApiState>()
		...
		var securityData = await session.LoadAsync<OAuthApiSecurityData>(resource.Id, cancellationToken);
		if (securityData is null)
		{
			return false;
		}
	...
	private static async Task<OAuthApiDto> MapToDtoAsync(
		IQuerySession session,
		OAuthApiState state,
		CancellationToken cancellationToken)
	{
		var securityData = await session.LoadAsync<OAuthApiSecurityData>(state.Id, cancellationToken);
