    ...
    TsTranspiler tsTranspiler) : IMembershipEvaluator
{
        ...
        CancellationToken ct = default)
    {
        ...
        // propagate the caller's CancellationToken so external callers can
        // still cancel cooperatively before the lib-level deadline fires.
        // Let exceptions bubble up — the caller (recalculator) maps them to a
        // MembershipRecomputeFailedEvent so the UI can surface the error
        // instead of silently reporting "0 members".
        using var _stopOnCancel = ct.Register(jsEngine.Stop);
