namespace Modgud.Domain.PositionTerminals.Contracts.V1;
...
// start/end per position/terminal, refresh presence views, invalidate caches.
...
// security audit, never consumer data. The business actor is the POSITION.

/// <summary>A passkey tap opened a staffing session — the position is now
/// staffed on this terminal until it ends or hits the absolute ceiling.</summary>
public sealed record PositionStaffingSessionStarted(
...
public sealed record PositionStaffingSessionEnded(
    Guid PositionPrincipalId,
    Guid TerminalEnrollmentId,
    Guid StaffingSessionId,
...
public sealed record PositionTerminalStatusChanged(
    Guid PositionPrincipalId,
    Guid TerminalEnrollmentId,
