...
/// - <see cref="IEvent{T}"/> wrapper (required for Event Forwarding)
/// - ShouldSync check (early exit if no relevant changes)
/// - SaveChangesAsync (auto-called after sync)
/// - Structured logging
/// <para>
/// Usage: inherit and implement <see cref="ShouldSync"/> + <see cref="SyncAsync"/>.
/// Wolverine discovers the <c>Handle</c> method automatically.
/// </para>
/// </summary>
...
/// <summary>
/// Auto-registers Wolverine PublishMessage subscriptions for all
/// <see cref="ReferenceSyncHandler{TEvent}"/> implementations. Scans the assembly for
/// concrete handlers, extracts the TEvent type parameter, and routes each unique
/// event type to the "reference-sync" local durable queue.
/// </summary>
