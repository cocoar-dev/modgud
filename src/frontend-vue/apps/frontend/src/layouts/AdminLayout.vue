...
import { CoarSidebar, CoarMenu, CoarMenuItem, CoarSubExpand, CoarAvatar, CoarButton } from '@cocoar/vue-ui';
...
watch(
  ...
  (path) => {
    ...
    if (path.startsWith('/admin/oauth')) {
      openSections.value['OAuth / OIDC'] = true;
    }
  },
  { immediate: true },
);
...
<template>
  <div class="app-layout">
    <CoarSidebar class="app-sidebar">
      ...
      <CoarMenu borderless>
        <CoarSubExpand label="Administration" v-model:open="openSections['Administration']">
          <CoarMenuItem
            label="Users"
            icon="users"
        ...
        <CoarSubExpand label="OAuth / OIDC" v-model:open="openSections['OAuth / OIDC']">
          <CoarMenuItem
            label="Clients"
            icon="key-round"
