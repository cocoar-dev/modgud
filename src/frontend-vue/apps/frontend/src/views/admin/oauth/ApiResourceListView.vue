...
const resources = ref<OAuthApiResource[] | null>(null);
...
builder
  ...
  .onRowClicked((event: any) => {
    if (event.data?.id) router.push(`/admin/oauth/api-resources/${event.data.id}`);
  });

// Set UI state synchronously (before first render)
ui.set(ctx => {
  ctx.header.title = 'API Resources';
  ctx.header.subTitle = 'Manage OAuth 2.0 API resources';
...
onMounted(async () => {
  try {
    const result = await adminApi.getOAuthApiResources();
    resources.value = result.items;
  } catch {
    error.value = 'Failed to load API resources.';
  } finally {
...
<template>
  <div class="list-page">
    <div class="page-actions">
      <CoarButton variant="primary" @click="router.push('/admin/oauth/api-resources/create')">New Resource</CoarButton>
    </div>
