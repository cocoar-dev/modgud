...
/// </summary>
public sealed class TimeTodoWebApplicationFactory : WebApplicationFactory<Program>
{
    private IHost? _host;

    public TimeTodoWebApplicationFactory(SharedPostgresFixture fixture)
    {
    ...
    /// <summary>
    /// Creates a test customer via event stream and returns the CustomerView.
    /// </summary>
    public async Task<CustomerView> CreateTestCustomerAsync(
        ...
        bool isImportant = false)
    {
        ...
        var id = Guid.CreateVersion7();
        var @event = new CustomerCreatedEvent(id, name, isImportant);
        session.Events.StartStream<CustomerView>(id, @event);

        await session.SaveChangesAsync(TestContext.Current.CancellationToken);

        // Wait for async projection to create the CustomerView
        var store = scope.ServiceProvider.GetRequiredService<IDocumentStore>();
        await store.WaitForNonStaleProjectionDataAsync(TimeSpan.FromSeconds(10));

        var view = await session.LoadAsync<CustomerView>(id, TestContext.Current.CancellationToken);
        return view!;
    }

    /// <summary>
    /// Creates a test todo via event stream and returns the TodoView.
    /// </summary>
    public async Task<TodoView> CreateTestTodoAsync(
        string title = "Test Todo",
        string? description = null,
        TodoStatus status = TodoStatus.New,
        Guid? customerId = null,
        Guid? parentTodoId = null,
        Guid? createdById = null,
        bool isCritical = false,
        bool isAwaitingFeedback = false,
        List<Guid>? responsibleUserIds = null)
    {
        using var scope = Services.CreateScope();
        var session = scope.ServiceProvider.GetRequiredService<IDocumentSession>();
        ...
        var @event = new TodoCreatedEvent(
            ...
            status,
            customerId,
            responsibleUserIds ?? new List<Guid>(),
            parentTodoId,
            isCritical,
            isAwaitingFeedback,
        ...
        session.Events.StartStream<TodoView>(id, @event);

        if (parentTodoId.HasValue)
        {
            session.Events.Append(parentTodoId.Value, new TodoChildAddedEvent(parentTodoId.Value, id));
        }

        await session.SaveChangesAsync(TestContext.Current.CancellationToken);

        // Wait for async projection to create the TodoView
        var store = scope.ServiceProvider.GetRequiredService<IDocumentStore>();
        await store.WaitForNonStaleProjectionDataAsync(TimeSpan.FromSeconds(10));

        var view = await session.LoadAsync<TodoView>(id, TestContext.Current.CancellationToken);
        return view!;
    }

    /// <summary>
    /// Creates a test comment via event stream and returns the CommentView.
    /// </summary>
    public async Task<CommentView> CreateTestCommentAsync(
        string description = "Test comment",
        Guid? referencedItemId = null,
        string referencedItemType = "todo",
        Guid? createdById = null,
        bool markAsReadByCreator = true)
    {
        using var scope = Services.CreateScope();
        var session = scope.ServiceProvider.GetRequiredService<IDocumentSession>();

        var id = Guid.CreateVersion7();
        var resolvedCreatedById = createdById ?? Guid.Empty;
        var now = DateTime.UtcNow;

        var createdEvent = new CommentCreatedEvent(
            id,
            description,
            referencedItemId ?? Guid.Empty,
            referencedItemType,
            now,
            resolvedCreatedById);
        ...
        if (markAsReadByCreator && resolvedCreatedById != Guid.Empty)
        {
            events.Add(new CommentMarkedAsReadEvent(id, resolvedCreatedById, now));
        }

        session.Events.StartStream<CommentView>(id, events.ToArray());
        await session.SaveChangesAsync(TestContext.Current.CancellationToken);
        ...
        var view = await session.LoadAsync<CommentView>(id, TestContext.Current.CancellationToken);
        return view!;
    }
