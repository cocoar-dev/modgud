...
using TimeToDo.Authorization.Commands;
        ...
        groupGroup.MapGet("{id}/effective-members", async (ShortGuid id, IDocumentSession session) =>
            {
                ...
                {
                    var person = p as TimeToDo.Authorization.Principals.Person;
                    return new
                    {
                        Id = new ShortGuid(p.Id).ToString(),
                        Label = p.DisplayName,
                        Type = p.Type,
                        UserName = (p as TimeToDo.Authorization.Principals.Person)?.AccountName,
