...
using TimeToDo.Api;
using TimeToDo.Api.ExtensionMethods;
using TimeToDo.Authentication;
using TimeToDo.Authentication.ExtensionMethods;
using TimeToDo.Authentication.Api.Account;
using TimeToDo.Authentication.Api.Account.Services;
using TimeToDo.Api.Features.Admin;
using TimeToDo.Authentication.AuthLog;
using TimeToDo.Api.Features.Comments;
using TimeToDo.Api.Features.Customers;
using TimeToDo.Authentication.Api.Admin;
using TimeToDo.Authentication.Api.Admin.IdentityProviders;
using TimeToDo.Authentication.Api.ExternalAuth;
using TimeToDo.Api.Features.Dev;
using TimeToDo.Api.Features.Migration;
using TimeToDo.Api.Features.Groups;
using TimeToDo.Api.Features.Principals;
using TimeToDo.Api.Features.Roles;
using TimeToDo.Api.Features.ScriptTypes;
using TimeToDo.Api.Features.Shared;
using TimeToDo.Api.Features.Todos;
using TimeToDo.Api.Features.Users;
using TimeToDo.Api.Helper;
using TimeToDo.Domain.Common;
using TimeToDo.Authentication.Domain;
using TimeToDo.Infrastructure;
using TimeToDo.Authentication.Setup;
using TimeToDo.Authentication.Identity;
using TimeToDo.Authentication.Identity.ExternalAuth;
using TimeToDo.Authentication.Identity.ExternalAuth.Flavors;
using Wolverine;
...
try
{
    ...
    builder.Services.AddSession(options =>
    {
        ...
        options.Cookie.Name = "TimeToDo.Session";
    });
    ...
    builder.Services.AddAuthentication(IdentityConstants.ApplicationScheme)
        .AddCookie(IdentityConstants.ApplicationScheme, options =>
        {
            ...
            options.Cookie.SecurePolicy = builder.Environment.IsProduction()
                ? CookieSecurePolicy.Always
                : CookieSecurePolicy.None;
            options.Cookie.Name = "TimeToDo.Auth";
        ...
        .AddCookie(IdentityConstants.TwoFactorUserIdScheme, options =>
        {
            ...
            options.Cookie.SecurePolicy = builder.Environment.IsProduction()
                ? CookieSecurePolicy.Always
                : CookieSecurePolicy.None;
            options.Cookie.Name = "TimeToDo.2FA";
        ...
        .AddCookie(IdentityConstants.ExternalScheme, options =>
        {
            ...
            options.Cookie.SecurePolicy = builder.Environment.IsProduction()
                ? CookieSecurePolicy.Always
                : CookieSecurePolicy.None;
            options.Cookie.Name = "TimeToDo.External";
    ...
    // AddTimeTodoAuthorization inside AddInfrastructure. Only keep app-specific wiring here.
    builder.Services.AddScoped<IAdminNotifier, AdminNotifier>();
    builder.Services.AddScoped<IDemoSeedService, DemoSeedImportService>();
    ...
    // UseTimeTodoAuthentication() so Infrastructure stays unaware of Authentication.
    builder.Services.AddInfrastructure(conf.DbSettings.ConnectionString,
        options => options.UseTimeTodoAuthentication());

    // Auto-migrate existing documents to event streams on startup
    builder.Services.AddHostedService<EventStoreMigrationService>();

    // One-time identity migration: ensures every user has a UserName + ApplicationUser doc
    builder.Services.AddHostedService<IdentityMigrationService>();

    // Wolverine CQRS + Marten projection side effects
    builder.Host.UseWolverine(opts =>
    {
        opts.Discovery.IncludeAssembly(typeof(Program).Assembly);
        opts.Discovery.IncludeAssembly(typeof(TimeToDo.Authentication.Api.Admin.RecoveryCli).Assembly);
        opts.Discovery.IncludeAssembly(typeof(TimeToDo.Authorization.Commands.CreateGroupCommand).Assembly);
    ...
    app.UseMiddleware<TimeToDo.Authentication.Api.Account.TwoFactorEnforcementMiddleware>();
    ...
    app.MapAuthorizationSimulatorEndpoints("api");
    ...
    app.MapScriptTypesEndpoints("api");
    app.MapCustomersEndpoints("api");
    app.MapTodosEndpoints("api");
    app.MapTodoMaintenanceEndpoints("api");
    app.MapCommentsEndpoints("api");
    app.MapMigrationEndpoints("api");
    ...
    // ResourceRegistry is now instance-based and configured via AddTimeTodoAuthorization
    ...
    app.Lifetime.ApplicationStarted.Register(() =>
        TimeToDo.Infrastructure.Events.ProjectionSideEffects.Enabled = true);

    // Break-glass recovery CLI — run inside the container instead of starting Kestrel.
    //   dotnet TimeToDo.Api.dll recover <command> [args...]
    if (args.Length > 0 && args[0].Equals("recover", StringComparison.OrdinalIgnoreCase))
    {
        return await TimeToDo.Authentication.Api.Admin.RecoveryCli.RunAsync(
