...
/// external identity maps to an existing TimeToDo user (by link or, carefully,
        ...
        Guid? authenticatedUserId = null)
    {
        ...
        if (link is not null)
        {
            ...
            if (authenticatedUserId is { } authId && authId != link.UserId)
            {
                ...
                return ExternalLoginResult.Failed("Idp.LinkedToOtherUser",
                    "This identity is already linked to another TimeToDo account.");
            }
        ...
        if (emailTaken)
        {
            ...
            return ExternalLoginResult.Failed("Idp.EmailConflict",
                "A TimeToDo account with this email already exists. Please contact your administrator.");
        }
                ...
                if (!string.IsNullOrWhiteSpace(newEmail))
                {
                    ...
                    if (clashingUserId != Guid.Empty)
                    {
                        ...
                        return new ApplyUpdatesError(
                            "Idp.EmailConflict",
                            "The identity provider reports an email that is already used by another TimeToDo account.");
                    }
                }
