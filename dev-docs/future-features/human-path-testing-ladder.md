...
> **Status:** Plan captured 2026-06-05; execution started the same day on `test/cold-start-ladder`. The enabling `ColdStartFixture` + in-process CLI harnesses are built; Stage 1 (the CLI) is essentially complete (modularized + fully covered), and the first proven findings across Stages 0/2/4 are closed with regression tests (full suite green: 1073 unit + 288 integration). Remaining rungs are sequenced per the ladder below.
>
