...
> **Status:** Plan captured 2026-06-05; execution started the same day on `test/cold-start-ladder`. The enabling `ColdStartFixture` harness is built, and the first three proven findings are closed with regression tests (full suite green: 1073 unit + 266 integration). Remaining rungs are sequenced per the ladder below.
>
