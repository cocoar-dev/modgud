...
> **Status:** Plan captured 2026-06-05; execution started the same day on `test/cold-start-ladder`. The enabling `ColdStartFixture` + in-process CLI harnesses are built; Stage 1 (the CLI) is essentially complete (modularized + fully covered), the proven findings across Stages 0/2/3/4 are closed with regression tests, and Stage 3 has its login-API contracts covered (full suite green: 1073 unit + 298 integration). The remaining headline Stage-3 item is the Playwright password golden-path (UI). Other rungs are sequenced per the ladder below.
>
