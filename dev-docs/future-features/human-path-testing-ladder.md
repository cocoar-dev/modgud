...
> **Status:** Plan captured 2026-06-05; execution started the same day on `test/cold-start-ladder`. The enabling `ColdStartFixture` + in-process CLI harnesses are built, and the first proven findings across Stages 0/1/2/4 are closed with regression tests (full suite green: 1073 unit + 273 integration). Remaining rungs are sequenced per the ladder below.
>
...
> - **Stage 0 (partial) — cold-boot milestone assertions.** `ColdStartBootTests` proves a blank boot seeds the system realm with its domains + `PrimaryDomain`, the `modgud` + `control-plane` apps with a real permission catalog, the six standard OIDC scopes, and the Internal login provider — and that the realm is genuinely adminless (zero users, zero invites).
...
> - **Stage 1 (partial) — the CLI harness + the silent-`--realm` finding closed.** An in-process `CliHarness` drives the real `RecoveryCli.RunAsync` against a booted host and captures stdout/stderr/exit-code (no CLI restructuring was needed to test it — that remains a deferred cleanup). `RecoveryCli` now resolves the global `--realm` up front for tenant-scoped commands: a misspelled `--realm` fails with a clear message instead of a deep Marten `UnknownTenantIdException`, and an implicit `system` default is announced when more than one realm exists (the same silent-tenant class as Stage 4). Guarded by `RecoveryCliTests` — bootstrap-admin direct (→ the admin can actually log in), bootstrap-admin invite (→ a `PendingAdminInvite` + printed link), missing-`--email` exit code, the two `--realm` cases (one verified red), the realm-domain guards, and help/no-args/unknown-command exit codes. **Still open on this rung:** per-command coverage for the remaining ~9 commands, and the one-class-per-command modularization.
>
