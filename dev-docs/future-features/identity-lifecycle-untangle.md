...
> **Update 2026-05-29 — maßgeblicher Spec:** Diese v1-Richtung wurde im Design-Dialog vollständig zu Entscheidungen A–G ausgearbeitet → **[federation-v1-design](./federation-v1-design)** ist ab jetzt die Bau-Vorlage. Wichtige Präzisierung dort: **„Session = Lease" ist wörtlich** — *kein* Mid-Session-Verfall-Timer. Ein capturedAt-basierter Timer-Drop mitten in der gültigen Session wurde **verworfen**: man widerruft nur auf *Evidenz* (Re-Login/SCIM/Session-Ende), nicht auf *Annahme* (sonst verliert ein intakter SSO-User mitten drin „die halbe App" — bricht das OAuth-Vertrauensmodell). Der **durable-mit-Lease/decay** aus der Research-Sektion unten ist die **v2**-Richtung, wo der Lease durch *Reconciliation* (Login-FORCE / SCIM / Pull) erneuert wird — nicht durch einen blinden Timer. `capturedAt` wird in v1 gespeichert, aber nicht als Drop-Timer durchgesetzt.
