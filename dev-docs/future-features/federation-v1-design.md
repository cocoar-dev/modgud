...
### D.1 — JWT-access clients (v1.1, 2026-05-29) {#d1-jwt-clients}

Not every resource server can consume opaque reference tokens, so JWT-access clients must be able to federate too (reference stays the recommendation). The boundary is relaxed without weakening the hub rule:

- At **token issuance** (`BakeFederatedResourceAccessForJwtAsync`, `AuthorizationEndpoints`), for `AccessTokenType.Jwt` clients, `BuildResourceAccessAsync` is called with `ReadSessionGroupIds(principal)` — while the carrier is still on the principal — and the **result** (`resource_access`, i.e. the permissions/roles the RS is entitled to) is baked into the self-contained access token. The **carrier itself stays no-destination**: only the rendered output ships, so the hub boundary holds exactly as for reference tokens.
...
- **UserInfo** echoes the token's baked block for JWT clients (the carrier isn't reconstructable server-side) instead of recomputing a narrower carrier-less set → token and UserInfo agree.
- **Freeze-on-refresh** is automatic: the refresh token is a reference token carrying the frozen carrier, so each refresh re-bakes the same set.
- **Per-scope gating unchanged**: a block is baked only when `roles` or `permissions` is in scope (no `groups` scope in v1).

**Trade-off (ratified 2026-05-29):** for JWT clients the federated membership is frozen for the access-token lifetime and the session lease (decision E) is enforced at the **refresh boundary** rather than instantly — the same staleness JWT clients already accept for durable permissions. Clients that need instant, server-authoritative revocation of the federated overlay use reference tokens.
...
- **H** (v1.1, 2026-05-29) — JWT-access clients federate too: the rendered `resource_access` is baked into the access token at issuance (carrier stays no-destination); UserInfo echoes it; lease enforced at the refresh boundary instead of instantly. Reference stays the recommendation; instant revocation ⇒ reference. See [D.1](#d1-jwt-clients). (Prerequisite: the [reference-token validation hotfix](#reference-token-hotfix).)
