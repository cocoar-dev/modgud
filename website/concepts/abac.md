...
- **Roles** — coarse-grained `app:resource:action` permissions per app.
- **Resolution** — a single decision per `(user, app, permission)`, propagated via UserInfo claims and the distribution API.
