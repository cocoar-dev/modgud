...
Deleting the account stays a deliberate action here — a manifest apply never deletes one. The `Credentials` list is the desired set for the account: a credential missing from a present list is deleted at apply, shown beforehand as a red *Clients* entry in the plan; an absent list leaves them alone. For a realm admin the credentials section of this modal stages onto the [draft](./configuration-drafts) the same way — only **Rotate secret** acts at once. See [service accounts and their credentials](realm-provisioning#service-accounts-and-their-credentials) for the rules.
