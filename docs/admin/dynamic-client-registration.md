...
`AllowRememberConsent` does not apply to DCR clients — Modgud decides per request instead, from whether the client's identity is assured ([RFC 8252 §8.6](https://www.rfc-editor.org/rfc/rfc8252#section-8.6)). A remembered authorization skips the consent screen only when the redirect delivers the code somewhere only the real client can receive it: an `https` redirect URI on a non-loopback host, or a confidential client (`client_secret_basic`/`client_secret_post`, or `private_key_jwt`) redeeming with its own credential. A **public** client redirecting to loopback or a private-use URI scheme — the shape every native/CLI DCR client registers, such as Claude Code, VS Code, Zed or Cursor — sees the consent screen on every authorize, because any local process can catch a loopback redirect and redeem the code with its own PKCE verifier. Clients in that position avoid repeated prompts by retaining and refreshing the authorization they already obtained (typical pattern: the agent caches its own consent decision and reuses the refresh token instead of re-authorizing).
