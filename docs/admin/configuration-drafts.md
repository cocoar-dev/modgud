...
- Entities the manifest does not model (terminal slots and terminal-managed clients, the PageBuilder) are managed live in their own admin surfaces. Service accounts export/import with their credentials; the account's delete stays live. A scheduled job's configuration and the inbox retention policy stage (`Jobs`, `InboxSettings`); a deployment-wide *system* job shown on the control plane is not realm configuration and saves live, and *Run now* is an action.
