...
| Service-account deletion, terminal slots (device enrollment), suspending / resuming a position grant, activation tokens | Ceremonies with a second party or a state machine of their own — the manifest's `Grants` list says *who* is authorized; suspend/resume flips a grant's own state |
...
- Entities the manifest does not model (terminal slots and terminal-managed clients, scheduled-job configuration, inbox retention settings, the PageBuilder) are managed live in their own admin surfaces. Service accounts export/import with their credentials; the account's delete stays live.
