...
While a draft is checked out, a **staging bar** sits at the bottom of the admin area — the only bar there. It shows the draft's name with how many changes are staged and how many plan errors block the apply (click either to open the draft's review), plus **Discard**, **Park** and **Apply**. Discard and Apply both ask for confirmation; Apply stays disabled while the plan has errors or conflicts, and says why. On a narrow window, Discard and Park move into a `⋯` menu. Lists show the *merged* state in a **Draft** column — `New`, `Changed`, or `To be deleted` — and a list's delete action reads **Stage deletion**. Parked and shared drafts are counted next to *Configuration drafts* in the sidebar.
...
On a large realm you don't pick entities in a dialog — you collect them where the search and filters are: every admin grid's context menu offers **"Add to export selection"**. The collected entities appear behind an **Export selection** chip in the header (it survives navigation and reloads, per browser); its panel lists them, **Clear** empties it, and **Download as manifest…** opens the selective-export review pre-filled with your collection — showing only the selection plus its required references, with the full list one checkbox away.
- **Staged deletions in the workspace** — deleting a row from any grid stages that deletion onto the draft the same way an API caller does through `PUT .../drafts/active/deletions/{section}?key=…` (see [declarative provisioning](realm-provisioning#deleting-staged-deletions-in-a-draft)); the row turns red in the plan and the same protections apply.
...
Manifests follow the platform-wide [merge-patch write semantics](/reference/#write-semantics): a field absent from the JSON stays unchanged, an explicit `null` clears the stored value, and `[]` clears a list — see [apply: merge-patch, always additive](realm-provisioning#apply-merge-patch-always-additive). The admin modals stage cleared fields as explicit `null`s automatically.
...
- **An apply is always additive on the target** — it never deletes anything the manifest leaves out, partial or not; delete anything unwanted on the target explicitly, through a staged deletion.
...
Service accounts transfer **with their credentials** (`Credentials` under the account: client id, scopes, apps, token settings) but **never a secret** — a credential the target lacks is issued there with a fresh secret, shown once at apply next to the ordinary client secrets. The account itself is never staged-deleted (deleting one kills every credential it owns; that stays a deliberate action in the service-account admin). The `Credentials` list is the desired set: a credential missing from a present list is deleted at apply and shows as a red *Clients* entry in the plan; an absent list leaves them alone. See [service accounts and their credentials](realm-provisioning#service-accounts-and-their-credentials).
