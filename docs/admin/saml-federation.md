...
1. **Admin → Login Providers → Add provider.** A single modal opens
...
2. **Flavor** (header dropdown): pick one.
   ...
   Switching flavor in this modal swaps the Connection-tab schema and
   the pre-seeded attribute map; admin-typed Display Name / Description
   stay intact across switches.
3. **General** tab: enter Display Name (shown on the login page button),
   a **Slug**, + optional Description. The slug is a short, URL-safe
...
5. **Create.** Provider lands **disabled** (security default — admin
   ...
   General tab with copy buttons.
6. Copy the SP metadata URL + ACS URL from the General tab and paste them
...
7. Once the IdP is wired and you've verified an SP-initiated login round
   trip, click the **Disabled** badge in the modal's header to flip
   it to **Enabled**. Modgud refuses to enable a SAML provider without
...
fill it in the Connection tab BEFORE clicking Create and the provider
...
- **SP cert** (ours, per realm). Auto-generated on first SAML provider activation in a realm. The underlying rotation mechanism promotes the active cert to "previous" with a 30-day overlap window — SP metadata advertises both during that window so IdPs still validating against the previous key keep working — but there is no admin-facing action to trigger it yet; it is not currently exposed via the admin UI, an API endpoint, or the recovery CLI.
