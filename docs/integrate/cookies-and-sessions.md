...
| `Modgud.Device` | `Lax` | "This browser has completed a login here" — a random, signed device id that keeps the user's own devices in their own login-failure bucket (ADR 0020, see [Rate limits](../platform/rate-limits#password-login-device-aware-throttling)). No fingerprint, no tracking; issued on every successful interactive login | 90 days idle |
