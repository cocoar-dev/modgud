...
**Amendment (2026-09-24):** the manifest applier's `?prune=true` full-sync flag and its two-step `409 Manifest.ConfirmationRequired` confirmation handshake (added during the 0.14 betas) were removed before v0.14.0. An `apply` is now always an additive merge/upsert — it never deletes anything the manifest leaves out. Deleting an entity — via the admin UI or the API — goes only through this ADR's staged-deletion mechanism (`PUT`/`DELETE .../drafts/active/deletions/{section}?key=<key>`, shown in the plan, applied with the draft). See [Declarative Realm Provisioning](../admin/realm-provisioning#deleting-staged-deletions-in-a-draft).
